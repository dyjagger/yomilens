...
One ViewModel owns immutable UI state. The camera composable owns CameraX binding, viewport alignment, and focus control. Exactly one of three output modes is selected at a time. The camera occupies the full screen; a compact translucent control tray and status pills float above it. Results are placed beside normalized OCR bounds on the frozen frame, with an outline tying every label to its detected Japanese. Mode changes reuse the same scan. Permission denial, camera startup, capture, recognition, no recognized text, camera failure, model preparation, and translation failure are explicit states.
