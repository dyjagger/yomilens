...
- The requested Britannica chart passed through the real scan-guide crop and Furigana pipeline on a local Android 16 emulator. The crop detected `友`, `月`, `目`, and `色` and returned `とも`, `つき`, `め`, and `いろ`; `橋` and `花` were outside the guide crop for that framing.
